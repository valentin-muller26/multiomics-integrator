...
# --------------------------------------------------------------------------- #
#  Load model files from the specified directory, erroring if no manifest found
# --------------------------------------------------------------------------- #
...
for (rep_k in seq_len(N_REP)) {
    ...
    common_genes  <- Reduce(intersect, lapply(valid_simulation, function(batch) rownames(batch$counts)))
    ...
    out_name <- sprintf("%s_rep%02d_seed%d_scDesign3.h5ad",
                        args$donor_id, rep_k + args$start_replicate_count - 1, seed_k)
