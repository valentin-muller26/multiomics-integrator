...
simulate_from_model <- function(fit_obj, seed, batch_id) {
    ...
    cat("    simu_new output    :", nrow(counts), "peaks x", ncol(counts), "cells\n")
...
for (rep_k in seq_len(N_REP)) {
    ...
    common_peaks  <- Reduce(intersect, lapply(valid, function(r) rownames(r$counts)))
    merged_counts <- do.call(cbind, lapply(valid, function(r) r$counts[common_peaks, , drop = FALSE]))
    ...
    out_name <- sprintf("%s_atac_rep%02d_seed%d_scDesign3.h5ad",
                        args$donor_id, rep_k, seed_k)
    out_path <- file.path(args$outdir, out_name)
    sim_ad$write_h5ad(out_path)
    cat("  Saved:", out_path, "\n")

    rm(sim_ad, merged_counts, merged_meta, valid, rep_results)
    invisible(gc())
}
