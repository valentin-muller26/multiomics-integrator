...
GROUP="$1"
SIMULATED_FILES_DIR="$PATH_DONOR_SUBSETS/simulated_data/RNA"
...
# Define paths for temporary files, output results, and graphs
...
if [[ -f "$TMP/${FILENAME}_ens.$EXTENSION" ]]; then
    ...
    python "$PATH_SCRIPTS/05b_mapmycell_convert_genename_to_ENS.py" \
        "$file" \
        "$TMP"
fi
...
if [[ -f "$OUTPUTDIR/$FILENAME.csv" ]]; then
    ...
    python -m cell_type_mapper.cli.map_to_on_the_fly_markers \
        ...
        --n_processors 4 \
        --cloud_safe False \
        --precomputed_stats.path "$PRECOMPUTED_STATS_FILE" \
...
python "$PATH_SCRIPTS/05b_analysis_mapmycells.py" \
    "$FILENAME" \
