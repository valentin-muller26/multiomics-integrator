...
h5ad file save in the output directory.
...
# QC stats from MyGene response
print(f"Queried: {len(a.var_names)} gene symbols")
print(f"  Not found in MyGene: {len(results['missing'])}")
print(f"  Multiple hits (took first): {len(results['dup'])}")
...
#Show statistic 
n_total = len(a.var_names)
n_mapped = a.var['ensembl_id'].notna().sum()
print(f"Mapped: {n_mapped} / {n_total}")
print(f"Unmapped: {n_total - n_mapped}")

# Safety check: warn if mapping rate is too low
mapping_rate = n_mapped / n_total
if mapping_rate < 0.5:
    print(f"WARNING: low mapping rate ({100*mapping_rate:.1f}%). Check input species/symbols.")
...
#Save the converted h5ad file
a_mapped.write_h5ad(output_file)
print(f"Converted h5ad file saved to: {output_file}")