#' Script to fit scDesign3 models for ATAC-seq data, batched by cell type
#'@param atacfile Path to the input h5ad ATAC-seq file for the ADNC group
#'@param outdir Output directory to save the fitted models (.rds files)
#'@param batch_size Number of cell types to include in each batch (default: 4)
#'@param cells_per_type Number of cells to sample per cell type (default: 20)
#'@param seed Random seed for reproducibility (default: 42)
#'@param var_quantile Quantile for variance filtering (default: 0.10, i.e., keep top 10% most variable peaks)
#' @param family_use Marginal distribution family ("nb", "zinb", or "zip")
#'@examples     
#' Rscript 03b_fit_model_scDesignATAC.R \
#'    --atacfile "/data/users/vmuller/0_master_thesis/data/SEAD_Dataset/patient_subsets/group/Not AD.h5ad" \
#'    --outdir "/data/users/vmuller/0_master_thesis/data/SEAD_Dataset/models/ATAC/Not AD"
...
parser$add_argument("--batch_size", type = "integer", default = 4,
                    help = "Number of cell types to include in each batch (default: 4)")
...
parser$add_argument("--var_quantile", type = "double", default = 0.10,
                    help = "Quantile for variance filtering (default: 0.90, i.e., keep top 10% most variable peaks)")
parser$add_argument("--model_family", type = "character", default = "nb",
                    choices = c("nb", "zinb", "zip"),
                    help = "Marginal distribution family: nb (default), zinb, or zip")
...
fit_batch <- function(keep_types, atac_counts, cell_type, donor_id,
                      cell_type_list, cells_per_type, seed, family_use) {
    ...
    #' @param atac_counts Matrix of ATAC-seq counts (peaks x cells)
    ...
    #' @param donor_id Named vector of donor IDs for each cell (names are cell barcodes) — needed as covariate in the model to capture differences between donors
    #' @param family_use Marginal distribution family ("nb", "zinb", or "zip")
    ...
    cells_keep <- unlist(lapply(keep_types, function(current_cell_type) { # external loop over each cell type in the batch
        ...
        donors       <- donor_id[colnames(atac_counts)[cell_indices]] # Get the donor IDs for those cells
        ...
        unlist(lapply(unique(donors), function(donor_name) { # internal loop over each donor for the current cell type
            ...
            if (n_take == length(donor_cell_indices)) { # If equal to the number of available cells for this donor, take all of them
                donor_cell_indices
            } else {
                donor_cell_indices[sample.int(length(donor_cell_indices), n_take)] # Otherwise, randomly sample n_take cells
            }
        }))
    }))
...
start_time <- Sys.time()
cat("Model family:", args$model_family, "\n\n")
# --------------------------------------------------------------------------- #
#  Load data
# --------------------------------------------------------------------------- #
...
# Sort by cell type
cell_barcode   <- colnames(atac_counts)
cell_order  <- order(cell_type[cell_barcode])
...
# Clean up peak_var to free memory before fitting the models
