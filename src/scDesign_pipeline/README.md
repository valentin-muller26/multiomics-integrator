...
This step is the second validation at the cell-type level (script `06_validate_simulation.R`, using the official validation script provided by the authors of scDesign3. Because this step is highly demanding in both memory and time, only five replicates per ADNC group and omics are evaluated. It assesses the statistical distributions of the real and simulated data, producing eight violin plots for the following eight summary statistics and comparing the distributions between the real and simulated data at four levels: feature level, cell level, feature-pair level, and cell-pair level:
...
Input required for the R scripts :
- sim_sce :path to the simulated files
- real_sce : path to the real count matrix of the ADNC group after feature pre-filtering generated during the fitting step (step 3)
- modality : indicates for which omics the validation is done for
- prefix : how the simulated files are name format (`<omics>_<ADNC_group>_rep<number>`
- output_dir : path to the output directory where the validation file will be generated

Output files:

- `<omics>_<ADNC_group>_rep<number>_cellmixs.tsv`: file containing the mLISI values for the PCA and UMAP
- `<omics>_<ADNC_group>_rep<number>_corr_data.tsv`: file containing the numerical values for the correlation test
- `<omics>_<ADNC_group>_rep<number>_rd.tsv`: file listing all the metadata for the specific replicate
- `<omics>_<ADNC_group>_rep<number>_RNA_panel.pdf`: PDF file containing the panel of all plots generated by the validation analysis
- `<omics>_<ADNC_group>_rep<number>_corr_heatmap.png`: correlation heatmap plot
- `<omics>_<ADNC_group>_rep<number>_summarystats.png`: violin plots of the eight summary statistics
- `<omics>_<ADNC_group>_rep<number>_pca.png`: PCA plot of real and simulated data
- `<omics>_<ADNC_group>_rep<number>_umap.png`: UMAP plot of real and simulated data
