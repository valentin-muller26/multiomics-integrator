#' Generate synthetic count replicates per ADNC group using scDesign3
#'
#' @description
...
#' the resulting count matrices are merged on their common genes, 
...
simulate_from_model <- function(fit_obj, seed, batch_id) {
    ...
    #'
    #' @description
    #' This function extracts NB parameters from the fitted model and runs the
    #' simulation with the given seed, renames the cells in the format
    #' (cell type + unique ID + batch ID), and builds the metadata for the
    #' obs field of the merged data. Errors are caught and logged, returning
    #' NULL so the calling loop can skip the batch without aborting the
    #' replicate.
    #'
    ...
    # Print the number of genes and cells for which parameters were extracted to understand the scale of the simulation
    cat(sprintf("    Fitted model: %d cells x %d genes\n", nrow(para$mean_mat), ncol(para$mean_mat)))
    ...
    # Print the dimensions of the simulated counts matrix (should correspond to genes x simulated cells)
    cat("    Simulated counts  :", nrow(counts), "genes x", ncol(counts), "cells\n")
...
for (rep_k in seq_len(N_REP)) {
    ...
    # Ensure that all batches have the same set of genes (rows) before merging
    common_genes  <- Reduce(intersect, lapply(valid_simulation, function(batch) rownames(batch$counts))) # retrieve the common genes
    merged_counts <- do.call(cbind, lapply(valid_simulation, function(batch) batch$counts[common_genes, , drop = FALSE])) 
